...
class SpeedLimitController:
  ...
  def update_limits(self, controlsState, dashboard_speed_limit, frogpilotCarState, gps_position, navigation_speed_limit, v_cruise, v_ego):
    ...
    if abs(desired_target - self.previous_target) >= 1:
      ...
      print("ℹ️ Source changed slightly — updating source only")
      self.source = desired_source
    else:
      print("⏱️ No significant changes — resetting timer")
      self.speed_limit_changed_timer = 0
      self.unconfirmed_speed_limit = 0
  ...
  def update_map_speed_limit(self, gps_position, v_ego):
    ...
    if self.next_speed_limit:
      ...
      if distance_to_upcoming < max_lookahead:
        map_speed_limit = self.next_speed_limit

    if map_speed_limit != self.map_speed_limit:
      self.map_speed_limit_timer += DT_MDL
      if self.map_speed_limit_timer >= 1:
